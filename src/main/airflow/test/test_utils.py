...
def test_setup(test_case_class,dag):
    ...
    uber_data_path, weather_data_path, app_path, app_dataset = get_app_configs(test_case_class.config, test_case_class.today)
...
def verify_schema(path, length, fields):
    ...
    schema = pq.read_table(files_list[0]).to_pandas().columns.values.tolist()
